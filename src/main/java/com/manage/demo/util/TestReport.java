...
import com.manage.demo.config.Config;
	...
	private void outputResult(List<ITestResult> fullResults, String path){
		...
		try {
			...
			for (int i = 0; i < fullResults.size(); i++)
			{
				...
				if (!StrUtil.isEmpty(fullResults.get(i).getMethod().getDescription()))
				{
			...
			for (int j = 0; j < skipArrayList.size(); j++)
			{
				...
				if (!StrUtil.isEmpty(skipArrayList.get(j).getMethod().getDescription())) {
			...
			for (int i = 0; i < passArrayList.size(); i++)
			{
				...
				if (!StrUtil.isEmpty(passArrayList.get(i).getMethod().getDescription())) {
			...
			for (int j = 0; j < failedArrayList.size(); j++)
			{
				...
				if (!StrUtil.isEmpty(failedArrayList.get(j).getMethod().getDescription())) {
